...
def _release_packing(model_to_pack, model_config, speaker, diff_to_pack, cluster_to_pack):
    return release_packing(
        os.path.join(ckpt_read_dir, model_to_pack),
        os.path.join(config_read_dir, model_config),
        speaker,
        os.path.join(diff_read_dir, diff_to_pack),
        os.path.join(ckpt_read_dir, cluster_to_pack))
...
parser.add_argument("--share", action='store_true', help='share', default=False)
...
with app:
    ...
    with gr.Tabs():
        with gr.TabItem("推理") as inference_tab:
            ...
            with gr.Row():
                diff_choice = gr.Dropdown(label="（可选）选择扩散模型", choices=diff_list, value="no_diff",
                                          interactive=True)
                diff_config_choice = gr.Dropdown(label="扩散模型配置文件", choices=diff_config_list,
                                                 value="no_diff_config", interactive=True)
        ...
        vc_tts_submit.click(tts_fn,
                            [text_input, tts_gender, tts_lang, tts_rate, tts_volume, output_format, sid, vc_transform,
        ...
        with gr.TabItem("训练"):
            ...
            gr.Markdown(
                value="""确认数据集正确识别后请选择训练使用的特征编码器和f0预测器，**如果要训练扩散模型，请选择Vec768l12或hubertsoft或whisper-ppg，并确保So-VITS和扩散模型使用同一个编码器**""")
            with gr.Row():
                ...
                """)
            with gr.Row():
                branch_selection = gr.Dropdown(label="选择训练使用的编码器", choices=encoder_list, value="vec768l12",
                                               interactive=True)
                f0_predictor_selection = gr.Dropdown(label="选择训练使用的f0预测器", choices=f0_options, value="crepe",
                                                     interactive=True)
            with gr.Row():
                use_diff = gr.Checkbox(label="是否使用浅扩散模型，如要训练浅扩散模型请勾选此项", value=True)
                vol_aug = gr.Checkbox(
                    label="是否启用响度嵌入和音量增强，启用后可以根据输入源控制输出响度，但对数据集质量的要求更高。**仅支持vec768l12编码器**",
                    value=False)
            with gr.Row():
                skip_loudnorm = gr.Checkbox(label="是否跳过响度匹配，如果你已经用音频处理软件做过响度匹配，请勾选此处")
                num_processes = gr.Slider(
                    label="预处理使用的CPU线程数，可以大幅加快预处理速度，但线程数过大容易爆显存，建议12G显存设置为2",
                    minimum=1, maximum=multiprocessing.cpu_count(), value=1, step=1)
            with gr.Row():
                raw_preprocess = gr.Button("数据预处理", variant="primary")
                regenerate_config_btn = gr.Button("重新生成配置文件", variant="primary")
            preprocess_output = gr.Textbox(label="预处理输出信息，完成后请检查一下是否有报错信息，如无则可以进行下一步",
                                           max_lines=999)
            ...
            with gr.Group():
                ...
                with gr.Row():
                    ...
                    gpu_selection = gr.Textbox(label="多卡用户请指定希望训练使用的显卡ID（0,1,2...）", value=gpus,
                                               interactive=True)
                with gr.Row():
                    log_interval = gr.Textbox(label="每隔多少步(steps)生成一次评估日志",
                                              value=sovits_params['log_interval'])
                    eval_interval = gr.Textbox(label="每隔多少步(steps)验证并保存一次模型",
                                               value=sovits_params['eval_interval'])
                    keep_ckpts = gr.Textbox(label="仅保留最新的X个模型，超出该数字的旧模型会被删除。设置为0则永不删除",
                                            value=sovits_params['keep_ckpts'])
                with gr.Row():
                    batch_size = gr.Textbox(
                        label="批量大小，每步取多少条数据进行训练，大batch有助于训练但显著增加显存占用。6G显存建议设定为4",
                        value=sovits_params['batch_size'])
                    lr = gr.Textbox(
                        label="学习率，一般不用动，批量大小较大时可以适当增大学习率，但强烈不建议超过0.0002，有炸炉风险",
                        value=sovits_params['learning_rate'])
                    # fp16_run=gr.Checkbox(label="是否使用fp16混合精度训练，fp16训练可能降低显存占用和训练时间，但对模型质量的影响尚未查证", value=sovits_params['fp16_run'])
                    amp_dtype = gr.Radio(label="训练数据类型，fp16可能会有更快的训练速度和更低的显存占用，但容易炸炉",
                                         choices=amp_options, value=sovits_params['amp_dtype'])
                    all_in_mem = gr.Checkbox(
                        label="是否加载所有数据集到内存中，硬盘IO过于低下、同时内存容量远大于数据集体积时可以启用，能显著加快训练速度",
                        value=sovits_params['all_in_mem'])
                with gr.Row():
                    # tiny_enable = gr.Checkbox(label="是否启用TINY训练，TINY模型的显存占用更低，推理速度更快，但质量有所削减。仅支持vec768的响度嵌入", value=False)
                    gr.Markdown(
                        "请检查右侧的说话人列表是否和你要训练的目标说话人一致，确认无误后点击写入配置文件，然后就可以开始训练了")
                    speakers = gr.Textbox(label="说话人列表")
            with gr.Accordion(label="扩散模型配置（训练扩散模型需要写入此处）", open=True):
                with gr.Row():
                    diff_num_workers = gr.Number(
                        label="num_workers, 如果你的电脑配置较高，可以将这里设置为0加快训练速度",
                        value=diff_params['num_workers'])
                    diff_k_step_max = gr.Checkbox(
                        label="只训练100步深度的浅扩散模型。能加快训练速度并提高模型质量，代价是无法执行超过100步的浅扩散推理",
                        value=diff_params['diff_k_step_max'])
                    diff_cache_all_data = gr.Checkbox(
                        label="是否缓存数据，启用后可以加快训练速度，关闭后可以节省显存或内存，但会减慢训练速度",
                        value=diff_params['cache_all_data'])
                    diff_cache_device = gr.Radio(
                        label="若启用缓存数据，使用显存(cuda)还是内存(cpu)缓存，如果显卡显存充足，选择cuda以加快训练速度",
                        choices=["cuda", "cpu"], value=diff_params['cache_device'])
                    diff_amp_dtype = gr.Radio(label="训练数据类型，fp16可能会有更快的训练速度，前提是你的显卡支持",
                                              choices=["fp32", "fp16"], value=diff_params['amp_dtype'])
                with gr.Row():
                    diff_batch_size = gr.Number(
                        label="批量大小(batch_size)，根据显卡显存设置，小显存适当降低该项，6G显存可以设定为48，但该数值不要超过数据集总数量的1/4",
                        value=diff_params['diff_batch_size'])
                    diff_lr = gr.Number(label="学习率（一般不需要动）", value=diff_params['diff_lr'])
                    diff_interval_log = gr.Number(label="每隔多少步(steps)生成一次评估日志",
                                                  value=diff_params['diff_interval_log'])
                    diff_interval_val = gr.Number(
                        label="每隔多少步(steps)验证并保存一次模型，如果你的批量大小较大，可以适当减少这里的数字，但不建议设置为1000以下",
                        value=diff_params['diff_interval_val'])
                    diff_force_save = gr.Number(
                        label="每隔多少步强制保留模型，只有该步数的倍数保存的模型会被保留，其余会被删除。设置为与验证步数相同的值则每个模型都会被保留",
                        value=diff_params['diff_force_save'])
            with gr.Row():
        ...
        with gr.TabItem("小工具/实验室特性"):
            ...
            with gr.Tabs():
                ...
                with gr.TabItem("模型发布打包/安装"):
                    ...
                    packing_btn.click(_release_packing,
        ...
        raw_preprocess.click(dataset_preprocess,
                             [branch_selection, f0_predictor_selection, use_diff, vol_aug, skip_loudnorm,
                              num_processes], [preprocess_output, speakers])
        ...
        save_params.click(save_default_settings,
                          [log_interval, eval_interval, keep_ckpts, batch_size, lr, amp_dtype, all_in_mem,
                           diff_num_workers, diff_cache_all_data, diff_cache_device, diff_amp_dtype, diff_batch_size,
                           diff_lr, diff_interval_log, diff_interval_val, diff_force_save, diff_k_step_max],
                          [write_config_output])
        write_config.click(config_fn, [log_interval, eval_interval, keep_ckpts, batch_size, lr, amp_dtype, all_in_mem,
                                       diff_num_workers, diff_cache_all_data, diff_batch_size, diff_lr,
                                       diff_interval_log, diff_interval_val, diff_cache_device, diff_amp_dtype,
                                       diff_force_save, diff_k_step_max], [write_config_output])
        ...
        with gr.Row(variant="panel"):
            with gr.Column():
                gr.Markdown(value="""
                    ...
                    """)
                with gr.Row():
                    debug_button = gr.Checkbox(label="Debug模式，反馈BUG需要打开，打开后控制台可以显示具体错误提示",
                                               value=debug)
                    read_second_dir = gr.Checkbox(
                        label="独立目录模式，开启后将从独立目录（./models）读取模型和配置文件，变更后需要刷新选项才能生效",
                        value=second_dir_enable)
        debug_button.change(debug_change, [], [])
        read_second_dir.change(webui_change, [read_second_dir], [])

        app.queue(concurrency_count=1022, max_size=2044).launch(server_name="127.0.0.1", inbrowser=False,
                                                                share=args.share)
